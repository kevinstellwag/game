  ...
  if (req.method === 'GET') {
    try {
      const r = await db.query(`
        ...
        ORDER BY CASE WHEN f.status='accepted' THEN 1 WHEN f.status='pending' THEN 2 ELSE 3 END, u.username
      `, [user.id]);
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      if (existing.rows[0]) {
        ...
        if (f.status === 'accepted') return err(res, 409, 'Jullie zijn al vrienden!');
        if (f.status === 'pending' && f.requester_id === t.id) {
          await db.query('UPDATE friendships SET status=$1 WHERE id=$2', ['accepted', f.id]);
          publish(userChannel(t.id), 'friend-accepted', { user: { id: user.id, username: user.username, color: user.color } });
          return ok(res, { status: 'accepted', user: { id: t.id, username: t.username, color: t.avatar_color } });
        }
        return err(res, 409, 'Vriendschapsverzoek al verstuurd');
      }

      await db.query('INSERT INTO friendships (requester_id,addressee_id) VALUES ($1,$2)', [user.id, t.id]);
      publish(userChannel(t.id), 'friend-request', { from: { id: user.id, username: user.username, color: user.color } });
      return ok(res, { status: 'pending', user: { id: t.id, username: t.username, color: t.avatar_color } });
    } catch (e) { return err(res, 500, e.message); }
  }
