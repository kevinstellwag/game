...
// Push game state to all players in parallel
async function pushToAll(players, sid, gs, maxPoints) {
  await Promise.all(players.map(p =>
    push(sessionChannel(sid), 'game-state', {
...
module.exports = async (req, res) => {
  ...
  const { action } = body;
  console.log('[game/id] sid:', sid, 'action:', action);
  ...
  try {
    ...
    console.error('[game/id] error:', e.message);
