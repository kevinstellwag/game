...
async function initDB() {
  await db.query(`
    ...
    CREATE INDEX IF NOT EXISTS idx_sessions_updated ON game_sessions(updated_at);
  `);
}

function hashPw(pw) {
  return new Promise((res, rej) =>
    crypto.scrypt(pw, 'cah_salt_v3', 64, (e, k) => e ? rej(e) : res(k.toString('hex')))
  );
}
...
// Ably — server-side REST client (no persistent connection needed)
function getAbly() {
  return new Ably.Rest(process.env.ABLY_API_KEY);
}

// Publish a message to an Ably channel
async function publish(channelName, eventName, data) {
  try {
    const ably = getAbly();
    const channel = ably.channels.get(channelName);
    await channel.publish(eventName, data);
  } catch (e) {
    console.error('[Ably publish error]', channelName, eventName, e.message);
  }
}

// Channel name helpers
function userChannel(userId)   { return `user-${userId}`; }
function sessionChannel(sid)   { return `session-${sid}`; }
...
module.exports = {
  ...
  publish, userChannel, sessionChannel,
  cors, parseBody, ok, err,
