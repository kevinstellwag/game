...
module.exports = async (req, res) => {
  ...
  if (!sessionId) return err(res, 400, 'sessionId verplicht');

  try {
    const r = await db.query('SELECT * FROM game_sessions WHERE id=$1', [sessionId]);
    ...
    if (action === 'start') {
      ...
      await db.query(
        ...
        [JSON.stringify(gs), sessionId]
      );
      await pushToAll(players, sessionId, gs, maxPoints);
    ...
    if (action === 'submit') {
      ...
      await db.query('UPDATE game_sessions SET game_state=$1, updated_at=NOW() WHERE id=$2', [JSON.stringify(gs), sessionId]);
      await pushToAll(players, sessionId, gs, maxPoints);
      return ok(res, { ok: true });
    }
